...
export function ClosingCta() {
  ...
  // A gradient can read var() directly, so the three stops below are tokens
  // rather than literals. The middle one used to be #10331e, a hex that existed
  // nowhere else in the project; it is now accent-900.
  return (
    <section
      ref={ref}
      ...
      style={{
        background:
          "linear-gradient(160deg, var(--color-ink-900) 0%, var(--color-accent-900) 60%, var(--color-accent-500) 130%)",
      }}
    >
      <div className="rounded-shell bg-bg-raised px-[20px] py-[80px] md:py-[120px]">
        <motion.div style={{ scale }} className="flex flex-col items-center">
          <MaskReveal
            ...
            className="max-w-[820px] justify-center text-center text-[36px] font-semibold leading-[1.02] tracking-[-0.02em] text-fg sm:text-[52px] lg:text-[68px]"
          />

          <Reveal delay={0.15}>
            <p className="mx-auto mt-6 max-w-lg text-center text-lg leading-relaxed text-fg-muted md:text-xl">
          ...
          <Reveal delay={0.3}>
            <div className="mx-auto mt-12 flex w-fit flex-wrap items-center justify-center gap-5">
              <MagneticCta
                ...
                target="_blank"
                rel="noopener"
                className="group relative isolate overflow-hidden text-base transition-transform duration-300 hover:-translate-y-0.5 md:!px-9 md:!py-4 md:text-lg"
              >
                <span
                  aria-hidden
