(() => {
  ...
  if (navSentinel && nav && "IntersectionObserver" in window) {
    const navObserver = new IntersectionObserver(
      (entries) => {
        ...
        if (entry) nav.classList.toggle("is-scrolled", !entry.isIntersecting);
      },
      { threshold: 0 },
    );
  ...
  const faqButtons = document.querySelectorAll<HTMLButtonElement>("[data-faq-toggle]");
  faqButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelectorAll<HTMLElement>("[data-faq-item].is-open").forEach((open) => {
